(function( $ ){
	...
	$.fn.multiple_emails = function(options) {
		...
		{
			deleteIconHTML = '<a href="#" class="multiple_emails-close" title="Remove"><span class="flaticon2-delete"></span></a>';
		}
		...
		return this.each(function() {
			...
			*/
			...
			function refresh_emails () {
				...
				//container.find('.multiple_emails-email span.email_name').each(function() { newEmails = newEmails +($(this).html()); });
				
				
				$orig.val(JSON.stringify(emails)).trigger('change');
				
				//console.log(newEmails);
				$("#current_emails").val(emails);
			}
		...
		$(function() {
			...
			$('#multipleEmails').multiple_emails({position: "bottom"});
			//OR $('#example_emailBS').multiple_emails("Bootstrap");
			
			//Shows the value of the input device, which is in JSON format
			$('#current_emailsBS').text($('#multipleEmails').val());
//			$('#example_emailBS').change( function(){
//				$('#current_emailsBS').text($(this).val());


			//Prevent key Enter input emails
			document.querySelector('.multiple_emails-input').addEventListener('keydown', function(e){
